...
Usage:
    ...
    output, has_error, stderr, images = await run_code("print(1 + 1)", language="python")
    output, has_error, stderr, images = await run_code(
        'name = input()\\nprint(f"Hello {name}")',
        language="python",
        user_input="World",
    )
...
VALID_LANGUAGES = {"python", "c", "cpp", "javascript", "java", "bash"}
...
async def run_code(
    ...
    Returns:
        A tuple of (output_text, has_error, stderr_text, images).
        ...
        ``images`` is a list of base64-encoded image strings generated by the code.
    ...
    # Automatically patch plt.show() to save an image instead
    if language == "python" and "plt.show()" in code:
        import uuid
        code = code.replace("plt.show()", f"import uuid; plt.savefig(f'output_{{uuid.uuid4().hex}}.png'); plt.clf()")
    ...
    async with stdio_client(_SANDBOX_PARAMS) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            with open("mcp_debug.log", "a") as f:
                f.write(f"IMAGES CAPTURED: {len(images)}\n")
                for i, img in enumerate(images):
                    f.write(f"IMAGE {i} LENGTH: {len(img)}\n")
                f.write("===============\n")
            ...
            return (output.strip() if output else "(no output)"), has_error, stderr_out, images
